                ...
                <div class="aui-money-list">
                    <c:forEach items="${records}" var="record">
                        <a href="javascript:;" class="aui-flex">
                            <c:if test="${record.type.equals('Park Fee')}">
                                <div class="aui-flex-box">
                                    <h2 style="color: #d12a2f">${record.amount}  <em>${record.type}</em></h2>
                                    <p>${record.timestamp} (${record.message})</p>
                                </div>
                            </c:if>
                            <c:if test="${record.type.equals('Transfer In')}">
                                <div class="aui-flex-box">
                                    <h2 style="color: #14d042">${record.amount}  <em>${record.type}</em></h2>
                                    <p>${record.timestamp} (${record.message})</p>
                                </div>
                            </c:if>
                            <c:if test="${record.type.equals('Transfer Out')}">
                                <div class="aui-flex-box">
                                    <h2 style="color: #d12a2f">${record.amount}  <em>${record.type}</em></h2>
                                    <p>${record.timestamp} (${record.message})</p>
                                </div>
                            </c:if>
                            <c:if test="${record.type.equals('Top Up')}">
                                <div class="aui-flex-box">
